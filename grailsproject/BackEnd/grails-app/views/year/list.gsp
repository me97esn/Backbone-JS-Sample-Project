			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="amount" title="${message(code: 'year.amount.label', default: 'Amount')}" />
					
						<g:sortableColumn property="yearfield" title="${message(code: 'year.yearfield.label', default: 'Yearfield')}" />
				...
				<g:each in="${yearInstanceList}" status="i" var="yearInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${yearInstance.id}">${fieldValue(bean: yearInstance, field: "amount")}</g:link></td>
					
						<td>${fieldValue(bean: yearInstance, field: "yearfield")}</td>
